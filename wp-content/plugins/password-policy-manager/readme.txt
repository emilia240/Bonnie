...
[Features](https://plugins.miniorange.com/password-policy-manager) | [Setup Guide](https://plugins.miniorange.com/setup-password-policy-manager-to-enforce-wordpress-password-security) | [Documentation](https://plugins.miniorange.com/password-policy-manager#free-demo)
...
* **[Users password manager:](https://plugins.miniorange.com/how-to-setup-password-policy-setting-wordpress-password-policy-plugin)** User password manager allows the admin to manage the users’ passwords (like password strength, how many passwords are strong, etc) to check the Password Security. [password policy setup guide]
...
* **[One click reset password:](https://plugins.miniorange.com/how-to-set-one-click-reset-password-setting-wordpress-password-policy-plugin)** This feature allows the admin to invalidate the current password and force their users to generate a new strong password. This can be done for all users in case of any breach. This will kill all the current sessions and users will be forced to set a new strong password via email hence reinforcing the Password Security.
* **[Password Score:](http://plugins.miniorange.com/how-to-set-password-score-or-password-strength-checker-password-policy)** It will show all the users’ password strengths. You can check whether the passwords being used are strong, medium or weak. Based on that you can use the enforce strong passwords feature to improve Password Security.
* **[Auto Password Expiry:](https://plugins.miniorange.com/how-to-setup-password-expiry-time-wordpress-password-policy-plugin)** This feature allows the admin to enforce a custom time-based password expiry to improve Password Security. Once the password has expired, the users will be forced to create a new password.
...
1. **Role-Based Enforce Strong Password on First Login:** Force specific users roles to create strong passwords according to the password policy set by the admin on their first login.
2. **User-Based One-click password Reset and Logout:** Admin can reset passwords of users at once and terminate their logged-in sessions with just one click in case of any suspicious activity using the One-click reset password. Admin can then send password reset links over email.
3. **Custom Redirect URL:** The admin can redirect their users to a different [custom URL](https://plugins.miniorange.com/how-to-set-custom-redirect-url-password-policy-manager-on-wordpress) using this functionality.
4. **Active & Inactive Users Activity Log:** The admin can track the activity of all active as well as lock inactive users using this tool of the Password Policy Enterprise plan.
5. **Logout Inactive Users:** When this setting is enabled, a user is logged out and their session is destroyed if they are inactive for more than the customizable set time limit.
6. **Multisite:** The Password Policy Manager Enterprise plugin is multisite compatible and can be used to create password policies for an entire multisite network.
...
>+ **[Role-Based and User-Based Password Policies:](https://plugins.miniorange.com/how-to-set-password-policy-role-based)** Admin can set different [role-based as well as user-based policies] and enforce password policy changes to ensure strong Password Security.
...
>+ **[Role-Based One-click password Reset and Logout:](https://plugins.miniorange.com/how-to-set-one-click-reset-password-policy-role-based)** Using [one-click reset password])  feature, the admin can reset passwords of all users / particular roles at once and terminate all logged-in sessions with just one click in case of any suspicious activity. Admin can then send the password reset link over email.
>+ **[Generate Random Passwords:](https://plugins.miniorange.com/how-to-generate-random-password)** Generate random passwords generates a random strong password containing all variations to make the password security strong and secure against brute force attacks.
>+ **[Automatically Lock Inactive Users:](https://plugins.miniorange.com/how-to-apply-automatically-lock-inactive-user)** It will lock the user automatically if the user is inactive for the custom-specified time period. This can be set for particular roles as well as users.
>+ **[Password History Manager:](https://plugins.miniorange.com/how-to-set-password-history-management)** It will manage the history of all the recently used passwords for each user, so no user can reuse a previous password.
...
1. Select "Password Policy Manager" from the WordPress left menu.
2. Configure features like strong passwords, password history, auto password expiry, and enforce them.
3. Save the settings to apply WordPress password policies site-wide.
...
1. Enforces strong and secure passwords site-wide.
2. Allows role-based password rules and expiration.
3. Supports one-click reset for all users.
4. Tracks password history and logout inactive users.
5. Offers better control over user password security than most plugins.
